...
(() => {
  ...
  // Keep chart endpoints aligned with the derived-metric engine: use the last
  // disclosed observation on or before the calendar anchor when it is within
  // the same declared tolerance.
  const rangeToleranceDays = {'3m':21,'6m':31,'ytd':45,'1y':45,'3y':62};
  const sources = [
  ...
  function historyWindow(curve, range='since', endDate=null) {
    ...
    const eligible=rows.filter(p=>end===null||stamp(p.date)<=end);
    let points=eligible;
    if(start!==null){
      let anchor=-1;
      for(let i=0;i<eligible.length;i++){if(stamp(eligible[i].date)<=start)anchor=i;else break;}
      const withinTolerance=anchor>=0 && start-stamp(eligible[anchor].date)<=rangeToleranceDays[range]*DAY;
      points=withinTolerance?eligible.slice(anchor):eligible.filter(p=>stamp(p.date)>=start);
    }
  ...
  function inceptionYears(rows, asOf=today()) {
    ...
    for(const row of rows||[]) {
      ...
      const start=row.inceptionDate||row.filterFacts?.firstDate;
      if(!String(start??'').trim()){missing++;continue;}
      const date=stamp(start);
