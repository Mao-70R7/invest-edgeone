(() => {
  ...
  let pointStyle=C.resolver(palette);
  let editingCompany='';
  ...
  function createMultiSelect(host, key, field, choices, title) {
    ...
    function refresh() {
      ...
      const matched=choices.filter(value=>C.label(value,field).toLocaleLowerCase('zh-CN').includes(query));
      ...
      const countByValue=new Map();C.filter(rows,{...state,[key]:[]}).filter(r=>F.matches(r,advanced,filterAsOf)).forEach(r=>{const v=String(r[field]||'').trim()||'__not_disclosed__';countByValue.set(v,(countByValue.get(v)||0)+1);});
  ...
  function savePalette(next, message) {
    palette=C.normalizePalette(next);pointStyle=C.resolver(palette);
  ...
  function renderScatter(filtered) {
    ...
    const accessible=`${metricLabel(state.xMetric)}与${metricLabel(state.yMetric)}。按基金公司配色，名称与数量见图例；当前选中使用深色外圈。筛选${filtered.length}只，可绘制${model.valid.length}只，视窗内${model.visible.length}只，视窗外${model.outside.length}只，指标缺失${missing.length}只。虚线采用全筛选样本中位数。`;
  ...
  function renderBusinessFilters(){
    const base=C.filter(rows,state),cache=new Map();
    ...
    const sourceCandidates=C.filter(rows,{...state,source:''}).filter(r=>F.matches(r,advanced,filterAsOf));
  ...
  document.getElementById('rankFilters').addEventListener('click',event=>{
    const benchmark=event.target.closest('[data-benchmark]');if(benchmark){F.setBenchmark(benchmark.dataset.benchmark);state.page=1;render({announce:true});return;}
