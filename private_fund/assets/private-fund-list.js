(() => {
  ...
  function createMultiSelect(host, key, field, choices, title) {
    ...
    function refresh() {
      ...
      options.innerHTML=visible.map(value=>`<label><input type="checkbox" value="${P.esc(value)}"${chosen.includes(value)?' checked':''}><span>${P.esc(C.label(value,field))}</span></label>`).join('')||'<p>没有匹配选项</p>';
