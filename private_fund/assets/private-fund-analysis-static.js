...
(() => {
  ...
  function catalog(onDownload) {
    catalogPromise ||= window.__PRIVATE_FUND_CATALOG__ ? Promise.resolve(window.__PRIVATE_FUND_CATALOG__)
      : dataScript(window.__PRIVATE_FUND_ANALYSIS_CATALOG_URL__ || './data/catalog.js.gz', 'window.__PRIVATE_FUND_CATALOG__=',onDownload).catch(e=>{catalogPromise=null;throw e;});
  ...
  function axisObservations(row, axis) {
    ...
    // Available-history values are evidence, never substituted into year-based ranks.
  ...
  async function build(params, progress=()=>{}) {
    ...
    return {target,products,coreElements:target.profile.coreElements,company:target.profile.company,managers:target.profile.managers,
      ...
      benchmark:{curve:[]},quality:{historySinceInception:false,targetCurvePointCount:targetCurve.length,benchmarkPointCount:0,volatilityExcludedCount:excludedVolatility,
        notes:[...(referenceKeys.size?['已补充标准参考产品；优先同风格同市场，其次同渠道及通用参考。参考池独立于严格同类池，不改变排名、中位数或目标评分。']:[]),...(peerLoadErrors.length?[`部分参考详情加载失败，已保留目标详情及其他可用参考：${peerLoadErrors.join('；')}`]:[]),...(target.incomeSeries.length?['货币型产品：万份收益以元/万份展示，七日年化以百分比展示；年化指标不代表实际持有期收益。']:[...(targetRow.dataQualityNote ? [`${targetRow.dataQualityStatus==='needs_review'?'待核对：':''}${targetRow.dataQualityNote}`] : []),pack.meta.comparability.warning, ...(targetRow.analysisSeriesLabel ? [`目标业绩口径：${targetRow.analysisSeriesLabel}；使用已入库可观测历史，不声明源端历史已全部披露。`] : []),'排名仅为当前分类与近1年窗口的研究参考；完整可用期收益不参加排名。夏普与索提诺无风险年利率假设2%。',
