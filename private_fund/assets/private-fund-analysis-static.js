...
(() => {
  ...
  const definitions = {
    ...
    annualReturn:{label:'成立以来年化收益',format:'pct',direction:'high'},
    annualVol:{label:'成立以来年化波动',format:'pct',direction:'low'},
    maxDrawdown:{label:'近1年最大回撤',format:'pct',direction:'high'},
    sharpe:{label:'成立以来夏普',format:'number',direction:'high'},
    sortino:{label:'成立以来索提诺',format:'number',direction:'high'},
    calmar:{label:'成立以来卡玛',format:'number',direction:'high'},
    monthlyWin:{label:'成立以来月胜率',format:'pct',direction:'high'},
    recovery:{label:'近1年回撤修复',format:'pct',direction:'high'},
  };
  const axes = [
    ['returnAbility','收益能力',['return1m','return3m','return6m','return1y'],'各区间收益的相对位置'],
    ['riskEfficiency','风险收益效率',['sharpe','sortino','calmar'],'成立以来夏普、索提诺与卡玛'],
    ['drawdownControl','回撤控制',['maxDrawdown'],'近1年最大回撤，越接近0越好'],
    ['volatilityControl','波动控制',['annualVol'],'成立以来年化波动，越低越好'],
    ['stability','盈利稳定性',['monthlyWin'],'成立以来正收益完整月份占比'],
  ...
  function catalog(onDownload) {
    catalogPromise ||= window.__PRIVATE_FUND_CATALOG__ ? Promise.resolve(window.__PRIVATE_FUND_CATALOG__)
      : dataScript(window.__PRIVATE_FUND_ANALYSIS_CATALOG_URL__ || './data/peers.js.gz', 'window.__PRIVATE_FUND_CATALOG__=',onDownload).catch(e=>{catalogPromise=null;throw e;});
  ...
  function profile(row, d) {
    ...
    Object.assign(core, {inceptionDate:row.inceptionDate, riskLevel:row.riskLevel,
      productScale:scale.scale_value_raw == null ? '未披露' : `${scale.scale_value_raw}${row.source==='simuwang' ? (!scale.scale_unit || ['source_unspecified','source_native','unverified','source_raw'].includes(scale.scale_unit) ? '（单位未核验）' : scale.scale_unit) : (scale.scale_unit_raw || '')}`,
  ...
  function scoringReason(row, pool, reference=false) {
    ...
    const reasons=[];
    if(!row.comparison?.eligible)reasons.push(row.metricMissing?.return_1y?.message||'近一年历史或有效月度样本不足。');
  ...
  function axisObservations(row, axis) {
    const metrics=row.comparison?.metrics||{}, values=axis.inputs.filter(c=>finite(metrics[c])).map(c=>({label:definitions[c].label,value:metrics[c],format:definitions[c].format,code:c}));
  ...
  async function build(params, progress=()=>{}) {
    ...
    // The cutoff must be close; since-inception metrics deliberately retain each
    // product's own available-history start instead of pretending starts align.
    ...
    const loadProduct=async row=>{
      ...
      const scores = Object.fromEntries(axes.map(a=>{const vals=a.inputs.map(c=>ranks[c]?.percentile).filter(finite);return[a.code,{score:vals.length?Math.round(vals.reduce((s,v)=>s+v,0)/vals.length*10)/10:null,usedInputs:vals.length,totalInputs:a.inputs.length,reason:vals.length?null:scoringReason(row,pool,referenceKeys.has(row.key)),observations:axisObservations(row,a)}];}));
      ...
      return {key:row.key,sourceId:row.source,sourceLabel:row.sourceLabel,productId:row.id,name:row.name,
        ...
        metrics:displayMetrics,metricSources,directMetrics:{},ranks,axisScores:scores,scoreNotice:scoringReason(row,pool,referenceKeys.has(row.key)),volatilityScreen:row.comparison?.volatilityScreen,
    ...
    return {target,products,coreElements:target.profile.coreElements,company:target.profile.company,managers:target.profile.managers,
      ...
      peerGroup:{mode,bucket:bucket(targetRow,mode),modes,count:pool.length,median,
        ...
        alignmentRule:'业绩截止日相差不超过7天。近1年收益、回撤和修复使用近1年；年化收益、年化波动及风险调整指标使用各产品成立以来完整可用历史，起点不强制一致。月度指标覆盖率至少80%。'},
      selectedComparison:custom&&products.some(p=>p.key===custom.key)?{key:custom.key}:null,metricDefinitions:definitions,axisDefinitions:axes,
      ...
      benchmark:{curve:[]},quality:{historySinceInception:false,targetCurvePointCount:targetCurve.length,benchmarkPointCount:0,volatilityExcludedCount:excludedVolatility,
        notes:[...(referenceKeys.size?['已补充标准参考产品；优先同风格同市场，其次同渠道及通用参考。参考池独立于严格同类池，不改变排名、中位数或目标评分。']:[]),...(peerLoadErrors.length?[`部分参考详情加载失败，已保留目标详情及其他可用参考：${peerLoadErrors.join('；')}`]:[]),...(target.incomeSeries.length?['货币型产品：万份收益以元/万份展示，七日年化以百分比展示；年化指标不代表实际持有期收益。']:[...(targetRow.dataQualityNote ? [`${targetRow.dataQualityStatus==='needs_review'?'待核对：':''}${targetRow.dataQualityNote}`] : []),...(targetRow.analysisSeriesLabel ? [`目标业绩口径：${targetRow.analysisSeriesLabel}；使用已入库可观测历史，不声明源端历史已全部披露。`] : []),'近1年收益、最大回撤与回撤修复按近1年计算；年化收益、年化波动、夏普、索提诺、卡玛及月胜率按各产品完整可用历史（成立以来）计算。成立以来起点可能不同，排名只作当前分类内研究参考。夏普与索提诺无风险年利率假设2%。',
          ...
          ...(targetRow.comparison?.eligible?pool.length<2?['当前分类与日期条件下不足2个可比样本；可切换分组或自选产品查看，不生成不足样本的排名。']:[]:['目标产品近1年区间或成立以来月度样本不足，保留已有数据，不生成可比排名。'])])]},
      meta:{generatedAt:pack.meta.generatedAt,databaseModifiedAt:pack.meta.generatedAt,queryDurationMs:Math.round(performance.now()-started),comparisonWindowLabel:'近1年收益/回撤；成立以来年化及风险调整指标',
  ...
  window.PrivateFundAnalysisStatic = {request,build,rank,bucket,profile,volatilityEligible,referenceCandidates,scoringReason,axisObservations};
})();
