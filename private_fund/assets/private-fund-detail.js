(() => {
  ...
  function render(detail) {
    ...
    const performanceAvailability = detail.performanceAvailability || {};
    const restrictedPerformanceCount = Number(performanceAvailability.restrictedMetricCount || 0);
    const performanceNotice = restrictedPerformanceCount > 0
      ? `<p class="analytics-note">${P.esc(performanceAvailability.message || "部分来源指标受访问权限限制，未作为日期或数值展示。")} 受限指标数：${restrictedPerformanceCount.toLocaleString("zh-CN")}。</p>`
      : "";
    ...
    const featuredCodes = [
      "derived_return_1y", "derived_max_drawdown_1y", "derived_annualized_return_since",
      "derived_annualized_volatility_since", "derived_sharpe_ratio_since",
      "derived_sortino_ratio_since", "derived_calmar_ratio_since", "derived_positive_month_ratio_since",
    ];
    ...
    const termsAvailability = detail.termsAvailability || {};
    const restrictedTermsCount = Number(termsAvailability.restrictedFieldCount || 0);
    const termsNotice = restrictedTermsCount > 0
      ? `<p class="analytics-note">${P.esc(termsAvailability.message || "部分产品条款受来源访问权限限制，未作为条款值展示。")} 受限字段数：${restrictedTermsCount.toLocaleString("zh-CN")}。</p>`
      : "";
    ...
    root.innerHTML = `
      ...
      <div class="detail-grid">
        <div class="detail-column">
          ...
          ${(performance.length || restrictedPerformanceCount) ? section("来源业绩指标", `${performanceNotice}${performance.length ? `<div class="metric-cards">${sourceMetricCards}</div><div class="table-wrap detail-table-wrap"><table class="detail-table"><thead><tr><th>指标</th><th>来源原值</th><th>截止日</th></tr></thead><tbody>${performanceRows}</tbody></table></div>` : ""}`) : ""}
          ...
          ${section("产品条款", `${termsNotice}${termsBody}`)}
        </div>
