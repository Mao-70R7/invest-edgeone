...
(() => {
  ...
  const fields=[
    ...
    ...[['return_1m','近1月收益率'],['return_3m','近3月收益率'],['return_6m','近半年收益率'],['return_ytd','今年以来收益率'],['return_1y','近一年收益率'],['return_2y','近两年收益率'],['return_3y','近三年收益率'],['return_5y','近五年收益率'],['return_since_inception','成立以来收益率'],['annual_return_1y','近一年年化收益'],['max_drawdown_1y','近一年最大回撤'],['volatility_1y','近一年年化波动率'],['sharpe_1y','近一年夏普比率']].map(([id,label])=>({id,label,group:'指标筛选',get:r=>metric(r,id),unit:id.startsWith('sharpe')?'倍':'%',bands:id.startsWith('max_drawdown')?[[-5,0],[-10,-5],[-20,-10],[null,-20]]:id.startsWith('sharpe')?[[0,1],[1,2],[2,null]]:id.startsWith('volatility')?[[0,5],[5,10],[10,20],[20,null]]:[[null,0],[0,10],[10,20],[20,30],[30,50],[50,null]]})),
    ...[['monthlyWin','近一年月胜率'],['sortino','近一年索提诺'],['calmar','近一年卡玛比率']].map(([id,label])=>({id,label,group:'指标筛选',get:r=>{const v=number(r.comparison?.metrics?.[id]);return v===null?null:v*(id==='monthlyWin'?100:1);},unit:id==='monthlyWin'?'%':'倍',bands:id==='monthlyWin'?[[0,50],[50,60],[60,80],[80,100]]:[[null,0],[0,1],[1,2],[2,null]]})),
    ...
    {id:'winning',label:'月胜率≥80%',group:'特色筛选',get:r=>{const n=number(r.comparison?.metrics?.monthlyWin);return n===null?null:n>=.8?'符合':'不符合';},note:'近一年可用月度观测，不等同于排排网专有标签。'},
  ...
  const legacy={annual:'annual_return_1y',drawdown:'max_drawdown_1y',volatility:'volatility_1y',sharpe:'sharpe_1y',monthlyWin:'monthlyWin',sortino:'sortino',calmar:'calmar'};
  const templates=Object.fromEntries(fields.map(f=>[f.id,f]));
  fields.splice(8);
  for(const [family,label,code,multiplier,note] of familySpecs){
    const ps=family==='return'?['since','ytd','1m','3m','6m','1y','2y','3y','5y']:family==='peerRank'?['ytd','6m','1y','2y','3y','5y']:family==='excessReturn'?['since','ytd','6m','1y','2y','3y','5y']:['since','1y','3y','5y'];
    const template=templates[family==='return'?'return_1y':legacy[family]||family];
    for(const period of ps){
      const id=family==='return'?(period==='since'?'return_since_inception':'return_'+period):period==='1y'?(legacy[family]||family):family+'_'+period;
      const f={...template,id,label:periods[period]+label,family,familyLabel:label,period,code,note};
  ...
  // AND rules observed on https://dc.simuwang.com/smph/ on 2026-09-15.
  const rules=[
    ['longTerm','长期业绩优异',5,[['rank5y','<=',50],['rank3y','<=',25],['max_drawdown_5y','<=',.5],['annualized_volatility_3y','<=',.3],['sharpe_ratio_3y','>',1]],'成立≥5年；5年收益同类前50%、3年前25%；5年回撤≤50%、3年波动≤30%、3年夏普>1。'],
    ['holding','持有体验佳',4,[['annualized_volatility_3y','<=',.1],['calendar3','>',0],['sharpe_ratio_3y','>',1]],'成立≥4年；3年波动≤10%；最近三个完整自然年各年收益>0；3年夏普>1。'],
    ['veteran','老牌基金经理',5,[['career','>',10],['rank3y','<=',30],['rank5y','<=',50],['max_drawdown_5y','<=',.5]],'成立≥5年；至少一位已披露经理从业>10年；3年收益同类前30%、5年前50%；5年回撤≤50%。'],
    ['highWin','高胜率',3,[['positive_month_ratio_3y','>',.7],['positive_month_ratio_1y','>',.8],['max_drawdown_3y','<=',.2],['rank3y','<=',50]],'成立≥3年；3年月胜率>70%、1年月胜率>80%；3年回撤≤20%；3年收益同类前50%。'],
    ...
    ['darkHorse','黑马潜力选手',2,[['rank2y','<=',20],['rank1y','<=',10],['max_drawdown_2y','<=',.3],['annualized_volatility_1y','<=',.2]],'成立≥2年；2年收益同类前20%、1年前10%；2年回撤≤30%；1年波动≤20%。'],
    ...
    ['lowVol','低波求稳',3,[['annualized_volatility_3y','<=',.2],['calendar3','>',0]],'成立≥3年；3年波动≤20%；2023至2025年各年收益>0。']
  ];
  ...
  function missing(f,r,asOf){
    ...
    const stem={return:'return',annual:'annual_return',drawdown:'drawdown',volatility:'volatility',sharpe:'sharpe'}[f.family],reason=r.metricMissing?.[stem+'_'+f.period]?.code;
  ...
  for(const f of fields){if(explanations[f.id])f.note=explanations[f.id];else if(f.feature)f.note='【组合加工】'+f.note+' 所列条件须同时满足。至少一项明确不满足，判为不符合；没有不符合但仍缺必要条件，判为条件不足。不是直接采集的推荐标签。成立日缺失时用首个净值日起算；成立以来指标使用完整可用历史，不保证覆盖真实成立日。同类名次使用本库共同月末窗口排名，并非排排网官方榜单；按细分策略、披露币种、业绩序列类型分组，端点允许相差7天，至少10个样本，备案号去重。经理从业年限使用已披露数值，多经理取最高值，未确认履历不推测。排名采用共同月末，其余风险指标截至各产品最新业绩日。';else if(f.group==='指标筛选')f.note='【'+(f.family==='peerRank'?'同类比较':'业绩加工')+'】'+f.note+' 【数据标准】优先按实际可用业绩计算；收益率无法计算时，仅采用日期、单位已明确的渠道披露值。排排网、格上、雪球采用复权业绩，其他渠道沿用已披露累计净值；不额外扣除或估算费用。最大回撤使用区间内全部已披露业绩点；波动率及风险调整比率采用月度样本，近一年至少10个月、近三年至少29个月、近五年至少48个月，且月度覆盖不少于80%；成立以来至少12个月。缺失不按0处理。';}
  ...
  function read(params){setBenchmark(params.get("benchmark"));const result={};for(const f of fields){const v=params.get('f_'+f.id);if(v&&v.length<200)result[f.id]=v;}return result;}
  function write(params,state){if(benchmark==='sh000300')params.delete('benchmark');else params.set('benchmark',benchmark);for(const f of fields){if(state[f.id])params.set('f_'+f.id,state[f.id]);else params.delete('f_'+f.id);}}
