...
(() => {
  ...
  function companySearchIndex(rows){
    const names=new Map();
    rows.forEach(r=>{const k=company(r);if(!names.has(k))names.set(k,new Set([k]));(r.companyAliases||[]).forEach(a=>names.get(k).add(a));});
    return new Map([...names].map(([k,v])=>[k,[...v].join(' ').toLocaleLowerCase('zh-CN')]));
  }
  ...
  function filter(rows, state) {
    ...
    return rows.filter(row => (!state.source || row.source === state.source)
      && (!companies.size || companies.has(company(row)) || (row.companyAliases||[]).some(n=>companies.has(n)))
      ...
      && (!query || normalize([row.name,row.fullName,row.id,row.registrationNumber,row.company,
        ...(row.companyAliases||[]),...(row.managers||[]),row.strategy1,row.strategy2,row.strategy3].join(' ')).includes(query)));
  }
  function readFilters(params, rows) {
    const aliases=new Map();rows.forEach(r=>(r.companyAliases||[]).forEach(a=>aliases.set(a,company(r))));
    return {companies:selection(params.getAll('company').map(n=>aliases.get(n)||n),values(rows,'company')),
  ...
  window.PrivateFundScatterControls={STORAGE_KEY,MISSING,GF_COMPANY,RED,GRAY,company,strategy,label,values,
    selection,defaults,normalizePalette,upsertRule,removeRule,resolver,filter,readFilters,writeFilters,companySearchIndex};
})();
