...
(() => {
  ...
  async function create(rows,key,root){
    ...
    return {get active(){return active;},apply(input){
      if(error)return [];
    ...
    },clear(){spec=null;error='';ids=null;active=false;missing=0;currentCode=null;const u=new URL(location.href);u.searchParams.delete('bf');history.replaceState(null,'',u);}};
  }
